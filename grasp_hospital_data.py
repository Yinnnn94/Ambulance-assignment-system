...
# hospital_dict = {'hospital_name': [], 'waiting_docs': [], 'waiting_hospitalization' : [] ,'whether_reported_119': [], 'update_time': []}
...
# print(hospital_name, whether_reported_119, waiting_docs, waiting_bed, waiting_hospitalization, update_time)
...
# title = ['更新時間','已向119通報滿床', '等待看診人數', '等待推床人數', '等待住院人數', '等待加護住院人數']
# td_list = []
# with open(hospital_log_path, 'a', encoding='utf-8') as f:
#     f.write('土城醫院')
#     f.write('\n')
#     td_list.append(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
#     for row in hospital_table[2:]:
#         for td in row.find_all('td'):
#             td_ = td.text.replace('\n', '')
#             td_ = td_.replace(' ', '')
#             if td_ != '':
#                 td_list.append(td_)
#     for t, td in zip(title, td_list):
#         f.write(t)
#         f.write(' ')
#         f.write(td)
#         f.write('\n')
